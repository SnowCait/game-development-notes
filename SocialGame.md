...
コラボなど他社 IP 関連のアセットディレクトリは独立させておく。（他プラットフォームや海外へ展開するときやコードを他プロジェクトへ流用する際に消せるように）
...
* DB は複数扱えるようにする
  * 特にログは MySQL, BigQuery, File など投げ先を変更したり複数に投げたりしがち
...
## キャッシュ
* 諸刃の剣
  * 1回目と2回目で通る処理が違うのでバグの温床になりやすい
    * テストを書きづらい
* ミドルウェアで一括適用するかなるべくフレームワーク（に近いコード）に任せる
* アプリケーションレイヤーに書く場合は最小限に留める
* 更新処理の判定や元データに使わない
...
## 多言語対応（ローカライズ）
* フレームワークの仕組みに乗っける
* 別サービスとしてリリースする場合でもインフラ構成はなるべく合わせておく（トラブル時の知見共有）
...
* 機能
  ...
  * ガチャ紹介
  * お知らせ
* 以前は負荷の兼ね合いと売上の観点から人の少ない時間帯（朝の4時〜6時頃）にやることが多かった
* 現在は負荷対策をしっかりやるので0時や昼間（12時〜15時）くらいにやるのが良さそう
  * 世界中に同一サーバーで配信するなら0時の意味はないので何かあったときに対応できる昼間が良さそう
  * 日本だけの配信なら0時にしておく方が考慮事項が減る
